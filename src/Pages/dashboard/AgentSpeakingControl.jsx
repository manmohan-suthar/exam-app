...
import { ChevronDown, ChevronRight, Eye, Users, Clock, AlertCircle } from 'lucide-react';
...
const AgentspeakingControl = () => {
  ...
  useEffect(() => {
    ...
    const fetchExam = async () => {
      ...
      if (!examData) {
        try {
          const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/exam-assignments/${examId}`);
      ...
      // Fetch paper
      try {
        const examPaper = examData.exam_paper || location.state?.exam?.exam_paper;
        ...
        const paperResp = await fetch(`${import.meta.env.VITE_API_BASE_URL}/speaking/${paperId}`);
  ...
  const selectPassage = (sectionIndex, passageIndex) => {
    ...
    if (socketRef.current) {
      socketRef.current.emit('change-section', { examId, section: sectionIndex });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <button onClick={() => navigate('/agent-dashboard')} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
            Back to Dashboard
          </button>
        </div>
      </div>
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className=" mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Agent speaking Exam Control</h1>
              <p className="text-gray-600">Student: {exam?.student?.name} | Exam: {paper?.title}</p>
            </div>
          ...
          <div className="flex gap-3">
            <button onClick={endExam} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg flex items-center gap-2">
              <AlertCircle size={16} />
              End Exam
            </button>
            <button onClick={() => navigate('/agent-dashboard')} className="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg">
              Back to Dashboard
            </button>
          </div>
        </div>
      ...
      <div className="mx-auto px-6 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1 bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <h3 className="text-lg font-semibold mb-4 text-gray-800">Exam Structure</h3>
                ...
                <div key={unitIndex} className="border border-gray-200 rounded">
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 flex items-center justify-between hover:bg-gray-50 ${
                      currentSection === unitIndex ? 'bg-blue-50 border-blue-200' : ''
                    }`}
                  >
                    <span className="font-medium">{unit.title}</span>
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-gray-500">({unit.passages.length})</span>
                      {expandedSections[unitIndex] ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                    </div>
                  </button>
                      ...
                      {unit.passages.map((passage, passageIndex) => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${
                            currentSection === unitIndex && currentPassage === passageIndex
          ...
          <div className="lg:col-span-2">
            {currentPassageData ? (
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div className="mb-4">
                  <h2 className="text-xl font-semibold text-gray-800 mb-2">
                    {currentPassageData.title || `Passage ${currentPassage + 1}`}
                  </h2>
                ...
                <div className="prose max-w-none mb-6">
                  <div
                    ...
                    dangerouslySetInnerHTML={{ __html: currentPassageData.content || currentPassageData.text || 'No content available' }}
                  />
                </div>

                {currentPassageData.images && currentPassageData.images.length > 0 && (
                  <div className="mb-6">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-gray-200">
                  <div className="flex gap-2">
                    <button
                      onClick={() => changeSection(Math.max(currentSection - 1, 0))}
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={currentPassage >= (currentUnit?.passages?.length || 0) - 1}
                      className="px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Next Passage
                    </button>
                    <button
                      onClick={() => changeSection(Math.min(currentSection + 1, units.length - 1))}
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">No Passage Selected</h3>
                <p className="text-gray-600">Select a section and passage from the sidebar to display content to the student.</p>
              </div>
            )}
          </div>
...
{connected && !isVideoCallActive && !videoCallError && (
  <div className="flex flex-col items-center justify-center flex-1 gap-6">
    <div className="text-center">
      ...
      <div className="text-sm text-gray-600">
        {studentConnected ? "Student connected. Starting video call automatically..." : "Waiting for student to join..."}
      </div>
    </div>
  </div>
)}
