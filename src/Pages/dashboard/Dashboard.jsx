...
const Dashboard = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (examType === 'listening') {
          // Construct audio URLs for listening exam
        ...
        // For speaking and reading exams, no additional examData needed
      } else {
        const errorText = await response.text();
  ...
  const testsData = assignments.flatMap((assignment) => {
    if (Array.isArray(assignment.exam_type)) {
      return assignment.exam_type.map((type) => ({
        ...
        description:
          assignment.exam_bio ||
          assignment.exam_paper?.[`${type}_exam_paper`]?.description ||
          "No Description",
        ...
        exam_paper: assignment.exam_paper?.[`${type}_exam_paper`],
  ...
  const isExamSkill = (skill) => {
    return (
      ...
      (Array.isArray(examAssignment.exam_type) ? examAssignment.exam_type.includes(skill.toLowerCase()) : examAssignment.exam_type.toLowerCase() === skill.toLowerCase())
    );
  };
