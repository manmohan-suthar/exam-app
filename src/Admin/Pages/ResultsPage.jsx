...
const ResultsPage = () => {
    ...
    if (listeningResult?.studentAnswers) {
      listeningResult.studentAnswers.forEach(answer => {
        if (answer.questionType === 'Blank_in_Space' && answer.adminMarked !== null) {
          existingDecisions[answer.questionNumber] = answer.adminMarked;
        }
      });
    }
  ...
  const handleAdminDecision = (questionNumber, isCorrect) => {
  ...
  const saveAdminDecisions = async () => {
    ...
    if (Object.keys(adminDecisions).length > 0) {
      ...
      if (listeningResult) {
        try {
          const decisionsArray = Object.entries(adminDecisions).map(([questionNumber, isCorrect]) => ({
          ...
          setResults(prevResults =>
            prevResults.map(result =>
              result._id === listeningResult._id
                ...
                {result.studentAnswers && result.studentAnswers.length > 0 ? (
                  <div className="grid gap-3">
                    {result.studentAnswers.map((answer, idx) => {
                      ...
                      if (result.examType === 'listening' && result.paper?.sections) {
                        ...
                        for (const section of result.paper.sections) {
                          if (section.questions) {
                            const question = section.questions.find(q => q.questionNumber === answer.questionNumber);
                            if (question) {
                        ...
                        const question = result.paper.questions.find(q => q.questionNumber === answer.questionNumber);
                        if (question) {
                          questionText = question.question || question.instructions || 'Question text not available';
                        }
                      } else if (result.examType === 'writing' && result.paper?.tasks) {
                        // Find task in writing tasks
                        const task = result.paper.tasks.find(t => t.taskNumber === answer.taskNumber);
                        if (task) {
                      ...
                      const adminMarked = adminDecisions[answer.questionNumber] !== undefined ? adminDecisions[answer.questionNumber] : answer.adminMarked;
                      const isCorrect = isBlankInSpace ? adminMarked : answer.userAnswer === answer.correctAnswer;

                      const currentWritingScore = writingScores[answer.taskNumber] !== undefined ? writingScores[answer.taskNumber] : (answer.adminScore || '');
                        ...
                        <div key={idx} className="p-4 bg-slate-50 rounded-lg border">
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex items-center space-x-3">
                              <span className="font-medium text-slate-700">
                                {isWritingTask ? `Task ${answer.taskNumber}` : `Q${answer.questionNumber}`}
                              </span>
                              {isBlankInSpace ? (
                                <div className="flex space-x-2">
                                  <button
                                    onClick={() => handleAdminDecision(answer.questionNumber, true)}
                                  ...
                                  <button
                                    onClick={() => handleAdminDecision(answer.questionNumber, false)}
                                    className={`px-3 py-1 rounded text-xs font-medium ${
                                ...
                                <div className="flex items-center space-x-2">
                                  ...
                                  <input
                                    ...
                                    max="9"
                                    step="0.5"
                                    value={currentWritingScore}
                                    onChange={(e) => handleWritingScoreChange(answer.taskNumber, e.target.value)}
                          ...
                          <div className="mb-3">
                            ...
                            <div className="bg-white p-3 rounded border text-sm whitespace-pre-wrap">
                              {isWritingTask ? answer.answer : answer.userAnswer || 'N/A'}
                            </div>
                            {isWritingTask && answer.wordCount && (
                              <p className="text-xs text-slate-500 mt-1">Word count: {answer.wordCount}</p>
                            )}
                          </div>
                          ...
                          {isWritingTask && answer.adminScore !== null && (
                            <div className="text-sm text-slate-600">
                              <strong>Current Score:</strong> <span className="font-medium">{answer.adminScore}/9</span>
                            </div>
                          )}
                        </div>
                      );
  ...
  if (selectedAssignment !== undefined) {
    ...
    const scoreBreakdown = ['listening', 'reading', 'writing', 'speaking'].map(examType => {
      ...
      return {
        ...
        gradedCount,
        totalCount,
        averageScore: totalCount > 0 ? Math.round((totalScore / totalCount) * 10) / 10 : 0
      };
    });
