...
const FillInBlankQuestion = ({ question, onUpdate, onDelete, index }) => {
  const [isEditing, setIsEditing] = useState(!question.question);
  const [editData, setEditData] = useState({
    question: question.question || '',
    fillOptions: question.fillOptions || [{ text: '' }],
  ...
  const handleSave = () => {
    ...
    const hasValidOptions = editData.fillOptions.some(opt => opt.text.trim());
    if (!hasValidOptions) {
      alert('Please enter at least one fill option');
      return;
    }
  ...
  const addOption = () => {
    setEditData({
      ...editData,
      fillOptions: [...editData.fillOptions, { text: '' }]
    });
  };

  const updateOption = (optionIndex, text) => {
    const newOptions = [...editData.fillOptions];
    newOptions[optionIndex] = { ...newOptions[optionIndex], text };
    setEditData({ ...editData, fillOptions: newOptions });
  };

  const removeOption = (optionIndex) => {
    if (editData.fillOptions.length <= 1) {
      alert('You must have at least one option');
      return;
    }
    const newOptions = editData.fillOptions.filter((_, idx) => idx !== optionIndex);
    setEditData({ ...editData, fillOptions: newOptions });
  };
    ...
    return (
      <div className="bg-white border border-slate-200 rounded-lg p-4">
        <div className="flex justify-between items-center mb-4">
          <h4 className="font-medium text-slate-800">Fill-in-the-Blank Question {index + 1}</h4>
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              rows={2}
              placeholder="Enter question or instruction (e.g., 'Complete the sentence with one word from the box')"
            />
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="block text-sm font-medium text-slate-700">
                Fill Options (shown on right side)
              </label>
              <button
                onClick={addOption}
            ...
            <div className="space-y-2">
              {editData.fillOptions.map((option, idx) => (
